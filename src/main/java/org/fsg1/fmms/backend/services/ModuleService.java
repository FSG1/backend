...
public class ModuleService extends Service {
    ...
    ModuleService(final Connection connection) {
        ...
        setQueryString(
                "WITH " +
                        "skills AS (SELECT Array_to_json(Array_agg(Json_build_object(" +
                        "'al', q.architecturallayer_id, 'ac', q.activity_id, " +
                        "'level', los.LEVEL))) AS json, lq.learninggoal_id FROM study.learninggoal_" +
                        "qualification AS lq inner join study.qualification AS q ON q.id = " +
                        "lq.qualification_id inner join study.levelofskill AS los ON los.id = " +
                        "q.levelofskill_id GROUP BY lq.learninggoal_id), " +
                        "lg AS (SELECT Array_to_json(Array_agg(Json_build_object('name', " +
                        "Concat('LG ', sequenceno), 'description', description, 'groupgoal', " +
                        "lg.groupgoal, 'skills', (SELECT json FROM skills WHERE " +
                        "skills.learninggoal_id = lg.id)))) AS json, lg.module_id FROM " +
                        "study.learninggoal AS lg GROUP BY module_id), " +
                        "activities AS (SELECT Array_to_json(Array_agg(" +
                        "Json_build_object('id', id, 'name', name, 'description', description))) " +
                        "AS json FROM study.activity), " +
                        "als AS (SELECT Array_to_json(Array_agg(Json_build_object('id', id, " +
                        "'name', name, 'description', description))) AS json FROM " +
                        "study.architecturallayer) " +
